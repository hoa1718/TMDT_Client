...
function Checkout() {
  ...
  const getProvince =()=>{
    axios.get("https://provinces.open-api.vn/api/?depth=1") .then(async (res) => {
      await setItems(res.data,"province");
    })
  }
  return (
    <>
            ...
            <div className="row">
              ...
              <div className="col-sm-4">
                <div className="order-message pay" style={{background:"#fafafa",padding:"0px 10px",borderRadius:"5px"}}>
                  ...
                  <div>
                    <span>Tạm tính:</span>
                    <span className="pull-right">{formatter.format(total)}</span>
                  </div>
                  <div>
                    <span>Điểm thưởng:</span>
                    <span className="pull-right">{formatter.format(point*10000)}</span>
                  </div>
                  <hr className="solid" style={{background:"black"}}/>
                  <div>
                    <span style={{fontSize:"20px"}}>Tổng cộng:</span>
                    <span style={{fontSize:"20px",color:"#fe980f"}} className="pull-right">{formatter.format(total - point * 10000)}</span>
                  </div>

...
function Checkout() {
  ...
  const getShipping =()=>{
    axios
    .get("http://localhost:4000/KhachHang/DiaChi/"+ReactSession.get('user').IdTaiKhoan)
    .then(async (res) => {
      await setShipping(res.data);
      console.log(shipptring);
    });
  }
  const getProvince = () => {
    axios
  ...
  const checkOut = () => {
    ...
    if (name === undefined || phone === undefined || address === undefined || Number(provinceSel.value)===0){

...
function Checkout() {
  ...
  const getDistrict =(province)=>{
    ...
    const getWard =(district)=>{
      axios.get(`https://provinces.open-api.vn/api/d/${district}?depth=2`).then(async (res) => {
  ...
  const displayProvince=(list)=>{
    return Object.keys(list).map((item, i) => {
      return(
        <option value={province[item].code}>{province[item].name}</option>
      )
    })
  }
  const displayDistrict=(list)=>{
    return Object.keys(list).map((item, i) => {
      return(
        <option value={district[item].code}>{district[item].name}</option>
      )
    })
  }
  const displayWard=(list)=>{
    return Object.keys(list).map((item, i) => {
      return(
        <option value={ward[item].code}>{ward[item].name}</option>
      )
    })
  }
  const checkOut =()=>{
    const provinceChoose= document.querySelector("#province").textContent;
    const districtChoose= document.querySelector("#district").textContent;
    const wardChoose= document.querySelector("#ward").textContent;
    console.log(provinceChoose , districtChoose, wardChoose);
  }
  useEffect(()=>{
    getProvince()
  },[])
  return (
    <>
      ...
      <section id="cart_items">
        ...
        {!ReactSession.get("user") && <div className="register-req">
            <Link className="btn btn-primary" style={{ marginTop: "0px" }} to="/Login">
              Đăng nhập
            </Link>
          </div> }
            ...
            <div className="row">
              <div className="col-sm-4">
                <div className="shopper-info">
                  ...
                  <form>
                    <input type="text" placeholder="Họ tên người nhận" onChange={(e)=>{setName(e.target.value)}} />
                    <input type="text" placeholder="Số điện thoại" onChange={(e)=>setPhone(e.target.value)} />
                    <input type="text" placeholder="Địa chỉ" onChange={(e)=>setAddress(e.target.value)}></input>
                    <select onChange={(e)=>{getDistrict(e.target.value)}} id="province">
                      <option hidden value="">
                        ---Tỉnh/Thành---
                      </option>
                      {displayProvince(province)}
                    </select>
                    <select onChange={(e)=>{getWard(e.target.value)}} id="district">
                      <option hidden value="">
                        ---Quận/Huyện---
                      </option>

...
function Checkout() {
  ...
  const checkOut = () => {
    ...
    formData.append("TenNguoiNhan",name);
    formData.append("SDT",phone);
    formData.append("DiaChi",`${address},${wardSel.options[wardSel.selectedIndex].text},${
      districtSel.options[districtSel.selectedIndex].text
    },${provinceSel.options[provinceSel.selectedIndex].text}`);
    axios
      .post("http://localhost:4000/HoaDon/create",formData)
      .then(async (res) => {
